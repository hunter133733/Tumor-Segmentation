...
def overlay_segmentation(ax, img_slice, seg_slice, title, vmax=None, cmap_img="gray",
                         cmap_seg="tab10", alpha=0.5):
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Visualize NIfTI image with GT and predictions from 10%, 25%, and 90% training subsets."
    )
    parser.add_argument("--image", required=True, help="Path to original image .nii/.nii.gz")
    parser.add_argument("--gt", required=True, help="Path to ground-truth segmentation .nii/.nii.gz")
    parser.add_argument("--pred_10", required=True, help="Path to 10%% subset prediction .nii/.nii.gz")
    parser.add_argument("--pred_25", required=True, help="Path to 25%% subset prediction .nii/.nii.gz")
    parser.add_argument("--pred_90", required=True, help="Path to 90%% subset prediction .nii/.nii.gz")
    ...
    gt = load_nifti(args.gt)
    pred_10 = load_nifti(args.pred_10)
    pred_25 = load_nifti(args.pred_25)
    pred_90 = load_nifti(args.pred_90)

    # Basic sanity check that shapes match
    shape = img.shape
    for arr, name in [(gt, "gt"), (pred_10, "pred_10"), (pred_25, "pred_25"), (pred_90, "pred_90")]:
        if arr.shape != shape:
            raise ValueError(
                f"Shape mismatch: image {shape}, {name} {arr.shape}"
            )

    # Extract the same slice for all
    img_slice, idx = get_slice(img, axis=args.axis, index=args.slice_index)
    gt_slice, _ = get_slice(gt, axis=args.axis, index=idx)
    ...
    overlay_segmentation(
        ...
        img_slice,
        gt_slice,
        title=f"Ground Truth ({axis_name} slice {idx})",
        vmax=args.vmax,
        cmap_seg="Reds",
        alpha=0.5,
    )
    ...
    fig.suptitle("Segmentation Comparison Across Training Subsets", fontsize=14)
